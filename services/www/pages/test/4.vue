...
<template>
  <div class="bg-gray-100 dark:bg-gray-900 min-h-screen p-4">
    <nav class="flex justify-between items-center py-4">
      ...
      <button @click="refresh" class="px-4 py-2 bg-purple-600 text-white rounded-md hover:bg-purple-700 dark:bg-blue-500 dark:hover:bg-blue-600 dark:text-white focus:outline-none">Refresh</button>
    </nav>
...
<div class="mt-4 border-t pt-4">
  ...
  <ul class="mt-2 space-y-2">
    ...
    <li>
      ...
      <span class="text-gray-800 dark:text-gray-300">{{ data.episodes }}</span>
    </li>
    <li>
      <strong class="text-purple-600">Score:</strong>
      <span class="text-gray-800 dark:text-gray-300">{{ data.score }}</span>
    </li>
    <li>
      <strong class="text-purple-600">Status:</strong>
      <span class="text-gray-800 dark:text-gray-300">{{ data.status }}</span>
    </li>
  </ul>
</div>
      </div>
    <div class="break-all">
      <div class="mt-4">
        ...
        <div class="flex flex-wrap space-x-2 mt-2">
          <div v-for="type in data.types" class="bg-blue-200 text-blue-700 dark:bg-green-400 dark:text-green-900 rounded-full px-2 py-1">{{ type }}</div>
        </div>
      </div>
     ...
     <div class="mt-4">
        ...
        <div class="flex flex-wrap space-x-2 mt-2">
          <div v-for="platform in data.platforms" class="bg-blue-200 text-blue-700 dark:bg-green-400 dark:text-green-900  rounded-full px-2 py-1">{{ platform }}</div>
        </div>
      </div>
      ...
      <div class="mt-4">
        ...
        <div class="flex flex-wrap space-x-2 mt-2">
          <div v-for="genre in data.genres" class="bg-blue-200 text-blue-700 dark:bg-green-400 dark:text-green-900 rounded-full px-2 py-1">{{ genre }}</div>
        </div>
      </div>
      ...
      <div class="mt-4">
        ...
        <div class="flex flex-wrap space-x-2 mt-2">
          <div v-for="tag in data.tags" class="bg-blue-200 text-blue-700 dark:bg-green-400 dark:text-green-900 rounded-full px-2 py-1">{{ tag }}</div>
        </div>
      </div>
      ...
      <div class="mt-4">
        ...
        <div v-for="trailer_url in data.trailer_urls" :key="trailer_url">
          <NuxtLink :to="trailer_url" class="text-purple-600 hover:underline dark:text-yellow-300 dark:hover:text-yellow-500">{{ trailer_url }}</NuxtLink>
        </div>

        <h3 class="text-xl font-semibold mt-4">Info URLs:</h3>
        <div v-for="info_url in data.info_urls" :key="info_url">
          <NuxtLink :to="info_url" class="text-purple-600 hover:underline dark:text-yellow-300 dark:hover:text-yellow-500">{{ info_url }}</NuxtLink>
        </div>

        <h3 class="text-xl font-semibold mt-4">Video URLs:</h3>
        <div v-for="video_url in data.video_urls" :key="video_url">
          <NuxtLink :to="video_url" class="text-purple-600 hover:underline dark:text-yellow-300 dark:hover:text-yellow-500">{{ video_url }}</NuxtLink>
        </div>
      </div>
