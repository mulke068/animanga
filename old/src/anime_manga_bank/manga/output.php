                        ...
                        <table class="border-separate border-spacing-2 border-collapse w-full border border-slate-400 dark:border-slate-500 bg-white dark:bg-slate-800 text-sm shadow-sm">
                            <thead class="bg-slate-50 dark:bg-slate-700">
                                <tr>
                                    <!--th>user_id</th-->
                                    <!--th>manga_id</th-->
                                    <td class="border border-slate-300 dark:border-slate-700 p-4 text-slate-500 dark:text-slate-400"><!--?= $row['name'];?-->Bild</td>
                                    <th class="w-1/2 border border-slate-300 dark:border-slate-600 font-semibold p-4 text-slate-900 dark:text-slate-200 text-left">Manga</th>
                            ...
                            <tbody>
                                <?php
                                    ...
                                    $query = "SELECT * FROM $dbtab,status WHERE (user_id=$uid) AND ($dbtab.status_id=status.status_id) AND CONCAT(name,name2) LIKE '%$search%' ORDER BY CONCAT(name,name2) ASC";
                                        ...
                                        $query = "SELECT * FROM $dbtab,status WHERE (user_id=$uid) AND ($dbtab.status_id=status.status_id) ORDER BY $dbtab.date DESC, $dbtab.manga_id DESC";
                                        ...
                                        foreach($query_run as $row)
                                        {
                                            ...
                                            <tr>
                                                ...
                                                <td class="grid gap-2 border border-slate-300 dark:border-slate-700 p-4 text-slate-500 dark:text-slate-400">
                                                    ...
                                                    <form action="../data/manga.php?uid=<?=$_GET['uid'];?>&db=<?=$_GET['db'];?>" method="POST">
                                                        <button class="bg-red-500 hover:bg-red-700 px-5 py-2 text-sm leading-5 rounded-full font-semibold text-white" type="submit" name="delete" value="<?=$row['manga_id'];?>">Delete</button>
                                                    </form>
                                                </td>
                                            </tr>
                                            <?php
